...
export function runMigrations(): void {
  ...
  const createMigrationsTable = db.prepare(
    'CREATE TABLE IF NOT EXISTS migrations (migration_id varchar(36) PRIMARY KEY, dateRan datetime DEFAULT CURRENT_TIMESTAMP)'
  );
...
function migrate(db: Database, migrations: Record<string, Migration>): void {
  Object.keys(migrations).forEach((migrationId) => {
    const migrationPreviouslyRun = db
      .prepare('SELECT 1 FROM migrations WHERE migration_id = ?')
      .get(migrationId);
    ...
    if (!migrationPreviouslyRun) {
      ...
      const migrationFile = fs.readFileSync(
        `${path.dirname(require.main?.filename as string)}/migrations/sql/${
          migrations[migrationId].up
        }.sql`,
        'utf8'
      );
      ...
      db.prepare('INSERT INTO migrations (migration_id) VALUES (?)').run(
        migrationId
      );
    }
  });
