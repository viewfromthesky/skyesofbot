...
export const SaveBookmark: SlashCommand = {
  ...
  options: [
    {
      name: 'name',
      description:
        "The name of your bookmark (make it relevant so it's easy to find later). 1024 character limit.",
      required: true,
  ...
  run: async (client: Client, interaction: BaseCommandInteraction) => {
    ...
    if (
      bookmarkName.length <= bookmarkNameMaxLength &&
      bookmarkContent.length <= bookmarkContentMaxLength
    ) {
      const db = openDbConnection();

      // Find out if the user has less than 25 bookmarks first (apparently this is a limit Discord places on the select menu)
      const { c: numberOfBookmarks }: { c: number } = db
      ...
      if (numberOfBookmarks < 25) {
        const update = db
          .prepare(
            'INSERT INTO bookmarks (user_id, bookmark_name, data) VALUES (?, ?, ?)'
          )
          .run(user.id, bookmarkName, bookmarkContent);
        ...
        if (update.changes && update.lastInsertRowid) {
          ...
          await interaction.reply({
            ephemeral: true,
            content: `There was an error saving your bookmark. Contact ${getOperatorName(
              client
